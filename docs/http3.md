...
On Zig 0.17 master, `std.Io.net.IncomingMessage.control` and `OutgoingMessage.control` expose ancillary bytes and POSIX `std.Io` backends pass them to `recvmsg(2)` and `sendmsg(2)`. `std.Io` does not expose a public socket-option operation, so Causeway uses `std.posix.setsockopt` and explicit per-OS `cmsghdr` policy. Supported ABIs are Linux, macOS, and FreeBSD: Linux/macOS send IPv4 `IP_TOS` as `c_int`, FreeBSD sends it as `u8`, IPv6 uses `c_int`, and receive parsing accepts both `IP_TOS` and `IP_RECVTOS` plus one-byte or integer payloads. OpenBSD, NetBSD, DragonFlyBSD, and other targets return `error.EcnUnsupported`; Causeway does not claim generic POSIX support.

Selecting ECN is strict: endpoint initialization returns `error.EcnUnsupported` or `error.EcnSetupFailed` instead of pretending ECN is active. Ancillary buffers are bounded stack storage for each receive/send batch, not persistent per-packet allocation. The transport metadata API (`Connection.receiveDatagramWithMetadata`) remains backend-independent and is covered with injection tests. Linux is exercised natively in this repository; macOS and FreeBSD are cross-compiled against their target ABIs but still require native kernel loopback verification for a production support claim. Use a `std.Io` backend with working `recvmsg` control-data delivery; this project does not paper over an incomplete kqueue receive implementation. On a dual-stack IPv6 socket, ECN metadata is guaranteed only for native IPv6 traffic because the already-bound endpoint cannot safely change `IPV6_V6ONLY` or require IPv4 TOS metadata on every supported kernel.
