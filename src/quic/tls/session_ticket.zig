...
//!
//! This module does not integrate resumption or 0-RTT. Future issuance code
//! must independently generate a fresh random `ticket_age_add` and a unique
//! per-connection TLS `ticket_nonce`; neither is the AES-GCM envelope nonce.
...
pub const maximum_plaintext_length = 1 + 8 + 4 + 4 + 2 + psk_length + 1 + maximum_alpn_length + 2 + maximum_quic_parameters_length + 2 + maximum_application_length;
pub const maximum_ticket_length = 4 + nonce_length + maximum_plaintext_length + tag_length;

const format_version: u8 = 1;
const aad_label = "causeway tls session ticket v1";
...
pub fn Controller(comptime capacity: usize) type {
    ...
    return struct {
        ...
        pub fn seal(self: *Self, output: []u8, value: *const Plaintext) ![]u8 {
            ...
            const total = 4 + nonce_length + cursor + tag_length;
...
fn samplePlaintext() Plaintext {
    return .{ .lifetime = 100, .age_add = 0xaabbccdd, .cipher_suite = .AES_128_GCM_SHA256, .psk = &test_psk, .alpn = "h3", .quic_transport_parameters = "quic", .application = "app" };
}
...
fn makeAuthenticatedZeroLifetimeTicket(controller: anytype, output: []u8, key: *const Key, entropy: *TestEntropy, issued_at: u64) ![]u8 {
    ...
    var cursor: usize = 0;
    putByte(&plaintext, &cursor, format_version);
    putInt(u64, &plaintext, &cursor, issued_at);
    putInt(u32, &plaintext, &cursor, 0);
    putInt(u32, &plaintext, &cursor, 1);
    putInt(u16, &plaintext, &cursor, @intFromEnum(tls.CipherSuite.AES_128_GCM_SHA256));
    putBytes(&plaintext, &cursor, &test_psk);
    putVector8(&plaintext, &cursor, "h3");
    ...
    const ciphertext = output[4 + nonce_length ..][0..cursor];
    const tag: *[tag_length]u8 = @ptrCast(output[4 + nonce_length + cursor ..][0..tag_length]);
    Aes256Gcm.encrypt(ciphertext, tag, plaintext[0..cursor], controller.makeAad(&aad, key.id), nonce, key.secret);
    return output[0 .. 4 + nonce_length + cursor + tag_length];
}
