...
HTTP/3 is server-side and poll-driven over UDP. It supports bounded streaming requests/responses, concurrent handlers, CONNECT takeover with directional half-close, RFC 9221 QUIC DATAGRAM, RFC 9297 HTTP Datagrams and Capsule Protocol fallback, per-packet application-limited recovery, cross-space persistent-congestion detection, and opt-in validated ECN with a Linux ancillary-data backend. Server push, 0-RTT, and WebTransport are not yet implemented. See [`docs/http3.md`](docs/http3.md) for exact behavior and limits.
